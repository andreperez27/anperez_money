...
- **Relatórios**: abas "Recebido & horas", "Acordo trabalhista" e "Entradas x
  ...
  lançamento. "Acordo trabalhista": só `origem='historico_acordo'` — **fato fechado**
  (2021–2025) sem seletor de período (período derivado do próprio dado; o
  estado do período da página fica preservado entre abas), cards do total do
  acordo, **gráfico com uma barra por ano** e **lista colapsável por ano**
  (acordeão, um ano aberto por vez, com a lista cronológica dos depósitos). "Entradas x despesas": fluxo real
...
- [diario/2026-09-12.md](diario/2026-09-12.md) — Aba **"Acordo trabalhista"** finalizada: fato fechado (2021–2025, `historico_acordo`) sem seletor de período (período derivado do dado; estado da página preservado entre abas), `porAno` aditivo na lib, hook independente de período (cards do total, gráfico **uma barra por ano**, `anos` para a lista), **acordeão com um ano aberto por vez** (`ListaAnosAcordo`, `aria-expanded`) via nova prop `detalhes` do template — corrigido o bug em que o ano expandido mostrava só 1 depósito (CSS: `overflow:hidden` + `max-height` do container pai encolhiam o item flex; solução: container sem limite + **sublista com scroll interno 30vh**, padrão das listas roláveis); suíte Acordo 11/11. **Export PDF com contexto** da tela: categoria isolada só na aba "Por categoria" (seleção elevada e persistente, `TODAS_CATEGORIAS`), PDF do acordo com a **tabela completa de depósitos**, consolidado intacto sem filtro. **Backup/planilha com agendador do Windows**: `backup_supabase.mjs` (todas as tabelas → CSV + manifest, paginado, RLS), `gerar_recebidos_planilha.mjs` + `sincronizar_planilha_supabase.py` (escreve só linhas novas, Excel COM preservando pivôs com fallback openpyxl), bats pro Agendador e scripts npm; `backups/` e o JSON intermediário no `.gitignore`. RelatorioPdf 22/22, build ok.
...
- [diario/2026-09-21.md](diario/2026-09-21.md) — PWA no mobile: ícones com nomes versionados v2 (fura cache que ignorava query string), ícone recomposto com nome completo legível (maskable v4 dentro da área segura do círculo, short_name "ANPEREZ Money") e verificação automática de versão com faixa de atualização (`versao.json`)
- [diario/2026-09-22.md](diario/2026-09-22.md) — Modo Calendário no Planejamento (toggle Lista/Calendário, mês próprio via `listarPorPeriodo`, seletor de período oculto no modo calendário, semana seg–dom, grid rígido `minmax(0,1fr)` com ellipsis, tela cheia `100vw` furando o `CaberNaTela` de 720px, painel do dia com o próprio `Lancamentos` embutido, remoção das abas Cartões/Planejado x real) + consumo real com registro separado Gás/Água (1055 só com os dois, congelamento por item na reprojeção, banco sem corrupção) + importação do histórico da planilha (16 inseridas) + fatura fechada vale o real (divergência PF 1126,14 vs 984,99; tags [Fatura Fechada]/[Fatura em Aberto] na lista, no detalhe e nos Cartões; faturas no calendário)
- [diario/2026-09-23.md](diario/2026-09-23.md) — Aba "Consumos" em Relatórios (água/gás por mes_referencia, lib pura + suíte 8/8, componente próprio com todos/detalhe/métrica/lista, seletor sem Semana; template compartilhado intacto)
